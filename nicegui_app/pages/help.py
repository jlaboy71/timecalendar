            ...
            with content_container:
                ...
                with ui.element('div').classes('w-full').style('display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1rem;'):
                    for chapter in chapters:
                        with ui.card().classes('cursor-pointer shadow-md hover:shadow-xl transition-all h-full').style(f'border-left: 4px solid {PTO_GOLD};').on('click', lambda c=chapter['id']: show_chapter(c)):
                            with ui.card_section().classes('h-full'):
                                with ui.row().classes('items-center gap-3 mb-3'):
                                    ui.icon(chapter['icon'], size='2rem').style(f'color: {PTO_GOLD};')
        ...
        def show_chapter(chapter_id):
            ...
            with content_container:
                ...
                with ui.row().classes('items-center gap-2 mb-4'):
                    ui.label('Help').style(f'color: {PTO_GOLD}; cursor: pointer;').on('click', show_chapters)
                ...
                with ui.card().classes('w-full shadow-md'):
                    for article in chapter['articles']:
                        with ui.row().classes('w-full p-4 border-b last:border-0 items-center cursor-pointer').style('transition: background-color 0.2s;').on('click', lambda a=article['id'], c=chapter_id: show_article(c, a)):
                            ui.icon('article').style(f'color: {PTO_GOLD};').classes('mr-3')
        ...
        def show_article(chapter_id, article_id):
            ...
            with content_container:
                ...
                with ui.row().classes('items-center gap-2 mb-4'):
                    ui.label('Help').style(f'color: {PTO_GOLD}; cursor: pointer;').on('click', show_chapters)
                    ui.label('/').classes('opacity-50')
                    ui.label(article['chapter_title']).style(f'color: {PTO_GOLD}; cursor: pointer;').on('click', lambda c=chapter_id: show_chapter(c))
                    ui.label('/').classes('opacity-50')
                    ui.label(article['title']).classes('font-semibold')

                # Article content
                with ui.card().classes('w-full p-6 shadow-md'):
                    ui.markdown(article['content']).classes('prose max-w-none prose-headings:text-primary')
            ...
            with content_container:
                ...
                if not results:
                    with ui.card().classes('w-full p-8 text-center shadow-md'):
                        ui.icon('search_off', size='3rem').style(f'color: {PTO_GOLD}; opacity: 0.3;').classes('mb-2')
                            ...
                            with ui.row().classes('w-full p-4 border-b last:border-0 cursor-pointer').style('transition: background-color 0.2s;').on('click', lambda r=result: show_article(r['chapter_id'], r['article_id'])):
                                with ui.column().classes('flex-1'):
                                    with ui.row().classes('items-center gap-2'):
                                        ...
                                        ui.badge(result['chapter_title']).style(f'background-color: {PTO_GOLD} !important;').classes('text-xs')
                                    if result['snippet']:
