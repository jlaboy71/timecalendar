...
def render_stats_bar():
    ...
    try:
        ...
        with ui.card().classes('w-full mb-6 p-4').style(f'border-left: 4px solid {TJM_GOLD};'):
            ...
            with ui.card().classes('p-4').style('width: 260px; min-width: 260px;'):
                ...
                def render_section_list(sections, query, selected_section, content_display, icons, names):
                    ...
                    for section in sections:
                        ...
                        tile_style = f'border: 2px solid {TJM_GOLD}; background: rgba(201,162,39,0.15);' if is_selected else 'border: 1px solid rgba(255,255,255,0.1);'
                        with ui.card().classes('w-full cursor-pointer p-2').style(tile_style).on('click', create_click_handler(section)):
                            with ui.row().classes('items-center gap-2'):
                                ui.icon(icon_name, size='sm').style(f'color: {TJM_GOLD};' if is_selected else 'color: gray;')
                                ui.label(display_name).classes('text-sm').style(
                                    f'color: {TJM_GOLD}; font-weight: 600;' if is_selected else ''
                                )
...
def update_content_display(section: dict, content_display: dict):
    ...
    with content_display['element']:
        ...
        with ui.card().classes('w-full mb-4 p-4').style(f'border-left: 4px solid {TJM_GOLD}; background: rgba(201,162,39,0.1);'):
            with ui.row().classes('items-center gap-3'):
                ui.icon(icon_name, size='lg').style(f'color: {TJM_GOLD};')
                ui.label(section['title']).classes('text-xl font-bold').style(f'color: {TJM_GOLD};')
...
def render_upload_analyze_panel(current_user):
    ...
    with ui.card().classes('w-full mb-6').style(f'border-left: 4px solid {TJM_GOLD};'):
...
def render_policy_changes_panel(current_user):
    ...
    try:
        ...
        with ui.card().classes('w-full p-6 mb-6').style(f'border-left: 4px solid {TJM_GOLD};'):
            with ui.row().classes('items-center gap-3 mb-4'):
